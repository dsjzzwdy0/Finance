...
public class WordDictionary
{
	...
	private DictSegment dictionary;
	...
	public void init(Path configFile)
	{
		...
		synchronized (WordDictionary.class)
		{
			...
			try
			{
				stream = Files.newDirectoryStream(configFile,
			...
			catch (IOException e)
			{
				...
				// e.printStackTrace();
				System.err.println(
						String.format(Locale.getDefault(), "%s: load user dict failure!", configFile.toString()));
			}
		}
	}
	...
	{
		dictionary = new DictSegment((char) 0);
		freqs.clear();
	}
	...
	private String addWord(String word)
	{
		if (null != word && !"".equals(word.trim()))
		{
			...
			dictionary.fillSegment(key.toCharArray());
			return key;
		}
		else
			return null;
	...
	public void loadUserDict(Path userDict, Charset charset)
	{
		try
		{
			...
			System.out.println(
					String.format(Locale.getDefault(), "user dict %s load finished, tot words:%d, time elapsed:%dms",
