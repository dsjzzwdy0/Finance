...
public class App
{
	...
	/**
	 * Example of how to use directly the algorithms of the framework in order
	 * to perform classification. A similar approach can be used to perform
	 * clustering, regression, build recommender system or perform topic
	...
	public static void main(String[] args)
	{
		 ...
		 *
		 * - datumbox.configuration.properties: It defines for the default
		 * storage engine (required) -
		 * datumbox.concurrencyconfiguration.properties: It controls the
		 * concurrency levels (required) -
		 * datumbox.inmemoryconfiguration.properties: It contains the
		 * configurations for the InMemory storage engine (required) -
		 * datumbox.mapdbconfiguration.properties: It contains the
		 * configurations for the MapDB storage engine (optional) - logback.xml:
		 * It contains the configuration file for the logger (optional)
		 */
		...
		Configuration configuration = Configuration.getConfiguration(); // default
																		...
																		// properties
																		// file
		// configuration.setStorageConfiguration(new InMemoryConfiguration());
		// //use In-Memory engine (default)
		// configuration.setStorageConfiguration(new MapDBConfiguration());
		// //use MapDB engine
		// configuration.getConcurrencyConfiguration().setParallelized(true);
		// //turn on/off the parallelization
		// configuration.getConcurrencyConfiguration().setMaxNumberOfThreadsPerTask(4);
		// //set the concurrency level

		// Reading Data
		// ------------
		Dataframe data;
		try (Reader fileReader = new BufferedReader(new InputStreamReader(new GZIPInputStream(new FileInputStream(Paths
				.get(App.class.getClassLoader().getResource("datasets/diabetes/diabetes.tsv.gz").toURI()).toFile())),
				"UTF-8")))
		{
