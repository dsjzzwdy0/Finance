	...
	for(var i = 0; i < len; i ++)
	{
		...
		if(yp.gid == gid && yp.source == source)
		{
			...
			if(source == sourceZgzcw)
			{
				...
				html += '<div class="handicap" title="' + yp.handicap + '">' + yp.handicap + '</div>';
				html += '<div class="oddsvalue" title="来源：' + source + '">' + yp.values[1].toFixed(2) + '</div>';
			}
			else
			{
				html = '<div class="oddsvalue" title="来源：' + source + '">' + (yp.values[0] - 1.0).toFixed(2) + '</div>';
				html += '<div class="handicap" title="' + yp.handicap + '">' + yp.handicap + '</div>';
				html += '<div class="oddsvalue" title="来源：' + source + '">' + (yp.values[1] - 1.0).toFixed(2) + '</div>';
			}			
			return html;
		}
	}
