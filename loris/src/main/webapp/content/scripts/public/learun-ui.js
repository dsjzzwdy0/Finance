...
Loading = function (bool, text) {
    ...
    //top.$("#loading_manage").css("left", 0);
    //top.$("#loading_manage").css("top", top.$('body').height() - top.$("#loading_manage").height());
    top.$("#loading_manage").css("top", 110);
}
...
dialogOpen = function (options) {
    ...
    var winObj = top;
    if (options.window) {
        winObj = options.window;
    }
    Loading(true);
    var _width = winObj.$.windowWidth() > parseInt(options.width.replace('px', '')) ? options.width : winObj.$.windowWidth() + 'px';
    var _height = winObj.$.windowHeight() > parseInt(options.height.replace('px', '')) ? options.height : winObj.$.windowHeight() + 'px';
    winObj.layer.open({
        id: options.id,
        type: 2,
        shade: options.shade,
...
$.currentIframe = function () {
    ...
    var $frame = top.$("#" + frameId);
    if ($frame.length > 0) {
        return $frame[0].contentWindow;
    } else {
        return top.$(".mainContent iframe")[0].contentWindow;//返回首页
    }
};
...
tabiframeId = function () {
    ...
    if (tabs_container.find('.on').attr('id') == undefined) {
        //代码应用于AdminLTE和adminWindows样式下。
        var tabs_container = top.$(".page-tabs-content");
        return "tabs_iframe_" + tabs_container.find('.active').attr('moduleid');
    }
    else {
        //代码应用于AdmindeFault下。
...
//在top层弹出对话框
function showPopWin1(Title, url, width, height, returnFunc, showCloseBox, Btn, isFresh) {
    //由于是在原有内容上新增Btn参数控制页面是否出现layer自带按钮，故默认不传此参数视为要带默认按钮，如果不要，请传入NO，非默认情况请输入自定义按钮如['确定']。
    var inbtn = ['确认', '关闭'];
    if (Btn != null) {
        if (Btn == 'NO') {
            inbtn = null;
...
//在当前窗口上弹出对话框
function showPopWin2(Title, url, width, height, returnFunc, showCloseBox, Btn) {
    //由于是在原有内容上新增Btn参数控制页面是否出现layer自带按钮，故默认不传此参数视为要带默认按钮，如果不要，请传入NO，非默认情况请输入自定义按钮如['确定']。
    var inbtn = ['确认', '关闭'];
    if (Btn != null) {
        if (Btn == 'NO') {
    ...
    dialogOpen({
        ...
        btn: inbtn,
        callBack: function (iframeId) {
            window.frames[iframeId].AcceptClick();
        }
    });
}

//替换原有jqueryUIDialog
function showPopContent(Title, content, width, height, showBtn) {
    dialogContent({
        window: window,
        id: null,
        title: Title,
