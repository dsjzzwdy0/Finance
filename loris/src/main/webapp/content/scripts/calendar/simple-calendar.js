class LunarHelp {
    constructor(year, month, day) {
        ...
        // 2018年3月7日增加， by Deng Shijun. 有一个小bug.
        if(i === 13)
        {
        	i = 1;
        	this.year += 1;
...
class SimpleCalendar {
    ...
    constructor(query, options) {
        ...
        this._defaultOptions = {
            ...
            height: '500px',
            language: 'CH',
            showLunarCalendar: true,            // 语言            
            showHoliday: true,					// 阴历
            showFestival: true,            		// 休假
            showLunarFestival: true,            // 节日
            showSolarTerm: true,            	// 农历节日
            showMark: true,            			// 节气
            onSelect: () => {},      	        // 标记
            onChange: () => {},
            timeRange: {
    ...
    */
    ...
    update(month = this.tmonth, year = this.tyear) {
        ...
        var calendar = this;
        if (typeof calendar._options.onChange === 'function') {
            calendar._options.onChange(year, month);
        }
    }
    ...
    updateEvent() {
        ...
        daysElement.forEach(function(v, i) {
            v.onmouseover = function(e) {
                if (this.classList.contains('sc-othermenth')) // 非本月的日期数
                {
                    return;
                }
            ...
            v.onclick = function() {
                ...
                if (this.classList.contains('sc-selected')) {
    ...
    updateDays(dataList) {
        ...
        daysElement.forEach(function(v, i) {
            ...
            for (var i = 0; i < len; i++) {
                var tradeDate = dataList[i];
                if (day == tradeDate.day) {
                    if (tradeDate.flag == false) {
                        ...
                        if (v.classList.contains('sc-selected')) {
                            v.classList.remove('sc-selected');
                        }
                    }
                }
            }
    ...
    getSelectedDays() {
        ...
        daysElement.forEach(function(v, i) {
            ...
            if (v.classList.contains('sc-selected')) {
                var day = v.querySelector(".day").innerHTML;
                if (day < 10) day = '0' + day;
                var date = selectYear + '-' + selectMonth + '-' + day;
                params.push(date);
            }
        });
        return params;
