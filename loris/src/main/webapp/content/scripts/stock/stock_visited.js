...
function stock_visited(){
	...
	var visited_futures = getCookie("VISITED_FUTURE") ? getCookie("VISITED_FUTURE").replace(/_0/g,"").replace(/_1/g,"").replace(/_2/g,"").split(",") : "";
	...
	if(visited_all){
		for(var key=0;key<visited_all.length;key++){
			...
			if(contains(visited_futures, "hf_" + visited_all[key])){
				visited_all[key] = "hf_" + visited_all[key];
			}
			if(contains(visited_futures, "CFF_RE_" + visited_all[key])){
				visited_all[key] = "CFF_RE_" + visited_all[key];
			}
		}
	...
	getData.getVisitedAll = function(){
		...
		function insert(){
				...
				for(var key=0;key<visited_all.length;key++){
					if(eval("hq_str_" + visited_all[key])){
						...
						if(contains(visited_futures, hq_str.code)){
							...
							hq_str.link = "http://finance.sina.com.cn/futures/quotes/" + hq_str.code + ".shtml";
							...
							//如果行情串和期货代码都以hf_开头
							if(hq_str.code.substr(0,3) == "hf_"){
								hq_str.code = hq_str.code.replace(/^hf_/,"");
								hq_str.link = "http://finance.sina.com.cn/futures/quotes/" + hq_str.code + ".shtml";
								hq_str.name = _hq_str[13];
								hq_str.price = _hq_str[0];
								hq_str.change = _hq_str[1];
							}
							//如果行情串和期货代码都以CFF_RE_开头
							if(hq_str.code.substr(0,7) == "CFF_RE_"){
								hq_str.code = hq_str.code.replace(/^CFF_RE_/,"");
								hq_str.link = "http://finance.sina.com.cn/futures/quotes/" + hq_str.code + ".shtml";
								hq_str.name = hq_str.code;
								hq_str.price = _hq_str[3];
								hq_str.change = (_hq_str[3] - _hq_str[14])*100/_hq_str[14];
							}
						}
						...
						html += "<tr class='"+hq_str.code+"'><td class='tableCellAlignLeft'>" + hq_str.type + "</td><td class='tableCellAlignLeft'><a href='" + hq_str.link + "' target='_blank'>" + hq_str.name + "</a></td><td class='tableCellAlignRight'>" + hq_str.price + "</td><td class='tableCellAlignRight'><span class='" + hq_str.chgclass+ "'>" + hq_str.change + "</span>" + hq_str.qrnh + "</td><td class='tableCellAlignCenter'><span class='del'></span></td></tr>";
						html_all += "<tr class='"+hq_str.code+"'><td class='tableCellAlignLeft'>" + hq_str.type + "</td><td class='tableCellAlignLeft'><a href='" + hq_str.link + "' target='_blank'>" + hq_str.code.toUpperCase() + "</a></td><td class='tableCellAlignLeft'><a href='" + hq_str.link + "' target='_blank'>" + hq_str.name + "</a></td><td class='tableCellAlignRight'>" + hq_str.price + "</td><td class='tableCellAlignRight'><span class='" + hq_str.chgclass+ "'>" + hq_str.change + "</span>" + hq_str.qrnh + "</td><td class='tableCellAlignCenter'><span class='del' style='margin:0 auto;'></span></td></tr>";
					}
				}
			}
			$(".stock_visited tbody").html(html);
			...
			$(".stock_visited table tbody td span.del").click(function(){
				$(this).parent("td").parent("tr").children("td").animate({"textIndent":"-200px","opacity":"0"},500,function(){
					visited_del($(this).parent("tr").attr("class"));
		...
		function visited_del(code){
			...
			for(var key in _visited_all){
				if(_visited_all[key] == "."+code || _visited_all[key] == code || _visited_all[key] == "00" + code || _visited_all[key] == "of" + code ){
