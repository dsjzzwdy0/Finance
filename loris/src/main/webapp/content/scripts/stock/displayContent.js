...
(function(){
    ...
    var FDC_DC=
    {
        ...
        displayContent:function(hashArr)
        {
            ...
            //ajax请求对应的txt文件
            var theResult=$.ajax({
            	url:"stock/page?symbol=sh600004", 
            	async:false
            });            
            
            //获取请求结果
            var blockJSON=theResult.responseText.replace(/[\r\n]/g,"");

            //向data_panel中填充html代码
            $("#data_panel").html(blockJSON);
            //$("#data_panel").html(theResult);
                            ...
                            if(/xggs|hgt-ggt/g.test(hash)&&(!/xggs-zs/g.test(hash)))
                           {
                                 ...
                                 $("#block_1 tbody tr").each(function(){
                                            ...
                                            var a_dom=document.createElement('a');
                                             ...
                                             $(a_dom).css({padding:'0 3px',backgroundColor:'#ffe036',color:'#9f4915',marginLeft:'5px',fontSize:'9px'});
                                             a_dom.innerHTML='LV2';
                                 ...
                                 { 
                                   ...
                                   $('#nav_tab').append('<a id="lev2_in_a" target="_blank" href="http://finance.sina.com.cn/data/hklevel2/order_confirm.html?link=hk" style="float: left;margin-top: 12px;padding: 4px;position:relative;font-size:14px;color:#0570b6;">订阅Level2行情<i style=" position: absolute;top: -4px;right: -21px;background:url(http://i2.sinaimg.cn/cj/data_center/new_20150116.png);width: 26px;height: 16px;"></i></a>');
                                         ...
                                         /*loginlayer.isLogin()?function(){
                                                    var has_money=$('.header div').is('#money_box');
                                                    if(has_money) $('div').remove('#money_box');
                                                   $('<div id="money_box" style="float: right;height: 80px;line-height: 80px;padding: 0 15px;font-size: 14px;"><a>我的账户</a></div>').insertBefore('#userLogin');
                                         }():function(){
                                                      $('div').remove('#money_box');
                                         }();*/
                            }
