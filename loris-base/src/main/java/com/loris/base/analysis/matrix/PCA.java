...
import com.loris.base.util.Assume;
...
{
	/**
	 * Type of the possible data transformation. ROTATION: rotate the data
	 * matrix to get a diagonal covariance matrix. This transformation is
	 * sometimes simply called PCA. WHITENING: rotate and scale the data matrix
	 * to get the unit covariance matrix
	 */
	...
	/** Where the input data should be standard. */
	private final boolean standardMatrix;
	 ...
	 * 
	 * @param data
	 *            data matrix used to compute the PCA transformation. Rows of
	...
	{
		this(data, new SVDBased(), true, true);
	}
	...
	{
		this(data, new SVDBased(), center, true);
	}
	 ...
	 *            covariance matrix
	 */
	public PCA(Matrix data, CovarianceMatrixCalculator evdCalc)
	{
		this(data, evdCalc, true, true);
	}
	 ...
	 *            calculations?
	 */
	public PCA(Matrix data, CovarianceMatrixCalculator evdCalc, boolean center, boolean standard)
	{
		/** Determine if input matrix should be centered */
		this.centerMatrix = center;
		this.standardMatrix = standard;
		
		/** Get the number of input dimensions. */
		this.inputDim = data.getColumnDimension();
		this.means = getColumnsMeans(data);
		this.stderrors = getColumnStderrors(data, means);
		...
		if(standardMatrix)
		{
			centeredData = starndardColumns(centeredData, stderrors);
		}
		...
		/**
		 * Get only the values of the matrices that correspond to standard
		 * deviations above the threshold
		 */
	...
	/**
	 * Get the means of the source Matrix.
	 * 
	 * @return
	 */
	...
	/**
	 * Get the Standard Errors of the Source Matrix.
	 * 
	 * @return
	 */
	...
	/**
	 * Get the matrix data that initialized. 
	 * 
	 * @return
	 */
	 ...
	 *            instances/samples, columns are dimensions. If the original PCA
	 *            data matrix was set to be centered, this matrix will also be
	 *            centered using the same parameters.
	 * @param type
	...
	private static Matrix starndardColumns(Matrix data, double[] standardError)
	{
		Assume.assume(standardError.length == data.getColumnDimension());
		Matrix m = new Matrix(data.getRowDimension(), data.getColumnDimension());
		for (int c = 0; c < data.getColumnDimension(); c++)
			for (int r = 0; r < data.getRowDimension(); r++)
			{
				if(standardError[c] == 0.0)
					m.set(r, c, 0.0);
				else
					m.set(r, c, data.get(r, c) / standardError[c]);
			}
		return m;
	}
	...
	private static double[] getColumnStderrors(Matrix m, double[] means)
	{
		double[] errors = new double[m.getColumnDimension()];
		for (int c = 0; c < m.getColumnDimension(); c++)
		{
			double sum = 0;
			for (int r = 0; r < m.getRowDimension(); r++)
				sum += Math.pow(m.get(r, c) - means[c], 2);
			errors[c] = Math.sqrt(sum / m.getRowDimension());
		}
		return errors;
	}
...
{
	/**
	 * Double machine precision in the R environment (i.e. in the R environment:
	 ...
	 * page of the R environment {@code prcomp} function (see the paragraph on
	 * {@code tol} parameter) it is written that in such setting we will "omit
	...
	/**
	 * @param tol
	 *            threshold parameter of the method - the same parameter as used
	 *            in R environment's `prcomp` function (see the paragraph on
	 *            {@code tol} parameter).
	 */
	 ...
	 * corresponds to the one in the `prcomp` function from the R environment
	 * (see the paragraph on {@code tol} parameter). The components are omitted
	 * if their standard deviations are less than or equal to {@code tol} (a
	 * parameter given in the constructor) times the standard deviation of the
	 * first component.
	 */
	...
	public Matrix getDAboveThreshold()
	{
		...
		Matrix newD = evd.d.getMatrix(0, aboveThresholdElemsNo - 1, 0, aboveThresholdElemsNo - 1);
		return newD;
	}
	...
	{
		return evd.v.getMatrix(0, evd.v.getRowDimension() - 1, getElementsNoAboveThreshold(),
				evd.v.getColumnDimension() - 1);
	}
