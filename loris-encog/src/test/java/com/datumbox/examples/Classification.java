...
public class Classification {
	...
	public static void testClass()
	{
        ...
        String filePath = "d:/index/encog/data/adult.csv";
        ...
        try (Reader fileReader = new BufferedReader(new InputStreamReader(new FileInputStream(new File(filePath)),
            ...
            headerDataTypes.put("fnlwgt", TypeInference.DataType.NUMERICAL);
            //headerDataTypes.put("education", TypeInference.DataType.CATEGORICAL);
            headerDataTypes.put("education-num", TypeInference.DataType.NUMERICAL);
            //headerDataTypes.put("marital-status", TypeInference.DataType.CATEGORICAL);
            //headerDataTypes.put("occupation", TypeInference.DataType.CATEGORICAL);
            //headerDataTypes.put("relationship", TypeInference.DataType.CATEGORICAL);
            //headerDataTypes.put("race", TypeInference.DataType.CATEGORICAL);
            //headerDataTypes.put("sex", TypeInference.DataType.CATEGORICAL);
            headerDataTypes.put("capital-gain", TypeInference.DataType.NUMERICAL);
            headerDataTypes.put("capital-loss", TypeInference.DataType.NUMERICAL);
            headerDataTypes.put("capital-loss", TypeInference.DataType.NUMERICAL);
            headerDataTypes.put("hours-per-week", TypeInference.DataType.NUMERICAL);
            //headerDataTypes.put("native-country", TypeInference.DataType.CATEGORICAL);
            headerDataTypes.put("income", TypeInference.DataType.CATEGORICAL);

            data = Dataframe.Builder.parseCSVFile(fileReader, "income", headerDataTypes, ',', '"', "\r\n", null, null, configuration);
      ...
      //Spit into train and test datasets
        ...
        for(Map.Entry<Integer, Record> entry: testingDataframe.entries()) {
            ...
            logger.info("Record "+rId+" - Real Y: "+r.getY()+", Predicted Y: "+r.getYPredicted());
        }
    ...
    public static void main(String[] args) { 
        ...
        //------------
        Dataframe data;
        try (Reader fileReader = new BufferedReader(new InputStreamReader(new GZIPInputStream(new FileInputStream(
        		Paths.get(Classification.class.getClassLoader().getResource("datasets/diabetes/diabetes.tsv.gz").toURI()).toFile())),
        		"UTF-8"))) {
            LinkedHashMap<String, TypeInference.DataType> headerDataTypes = new LinkedHashMap<>();
